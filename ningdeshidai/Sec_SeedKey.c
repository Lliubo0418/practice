...
static uint32 uds_PBKDF2GenerateRootKey(uint8 *geneKeyByte);
#if defined( SEC_ENABLE_SEED_KEY_ENTROPY_POOL )
...
#define SECURE_CFG_BARCODE_SALT                 ((uint8 *)(0xAF058000u) + 2u)
#define SECURE_CFG_BARCODE_SALT_BYTE_LEN        (16u)
#define SECURE_BARCODE_BYTE_LEN                 (12u)
#define SECURE_ROOTKEY_BYTE_LEN                 (16u)
#define SECURE_PW_BYTE_LEN                      (16u)
#define SECURE_CFG_ITERATION_CNT                (1u)
#define DIAT_BARCODE_ADDR                       (uint32)0xFF21F800u

static uint8 saltDefaultVal[4] = {0x72u, 0x33u, 0x58u, 0x16u};
static const uint8 PBKDF2_OriginalPW[SECURE_PW_BYTE_LEN] = 
{
    0x39,0x30,0x38,0x32,
    0x53,0x4D,0x42,0x32,
    0x70,0x66,0x74,0x53,
...
static const uint8 barcodeDeaultval[SECURE_BARCODE_BYTE_LEN] = 
{
    0x02,0x57,0x94,0x72,
    0x24,0x51,0x32,0x47,
    0x26,0x48,0x92,0x20
...
static uint32 uds_PBKDF2GenerateRootKey(uint8 *geneKeyByte)
{
    uint32 result = SECM_OK;
    uint8 salt[SECURE_CFG_BARCODE_SALT_BYTE_LEN] = {0u};
    eslt_ErrorCode retVal = ESL_ERC_NO_ERROR;
    Std_ReturnType barcodeResult = E_OK;

    /* #10 creat the workspace for PBKDF2 calculation and define watchdog funciton*/
    eslt_WorkSpaceKDF2HMACSHA256 udsPBKDF2Ws;

    /* #20 copy barcode as salt value*/
    VStdLib_MemCpy(&salt[12u], saltDefaultVal , (SECURE_CFG_BARCODE_SALT_BYTE_LEN-SECURE_BARCODE_BYTE_LEN));

    barcodeResult = Diat_GetBarcodeDataSeclevel(&salt[0u]);
    if (barcodeResult == E_NOT_OK)
    {
        VStdLib_MemCpy(&salt[0u], barcodeDeaultval, SECURE_BARCODE_BYTE_LEN);
    }

    /* #30 PBKDF2 function initialization*/
    retVal = esl_initWorkSpaceHeader(&udsPBKDF2Ws.header,ESL_MAXSIZEOF_WS_KDF2HMACSHA256,FblLookForWatchdogVoid);
    
    if(retVal == ESL_ERC_NO_ERROR)
    {
        retVal = esl_initKDF2HMACSHA256(&udsPBKDF2Ws,SECURE_CFG_ITERATION_CNT);
    }
    
    /* #40 PBKDF2 function Calculation,the result is geneKeyByte with byte format*/
    if(retVal == ESL_ERC_NO_ERROR)
    {
        retVal = esl_deriveKeyKDF2HMACSHA256(   &udsPBKDF2Ws,
                                                SECURE_PW_BYTE_LEN,        &PBKDF2_OriginalPW[0u],
                                                SECURE_CFG_BARCODE_SALT_BYTE_LEN,       &salt[0u],
                                                SECURE_ROOTKEY_BYTE_LEN,         &geneKeyByte[0u]);
    }

    result = (retVal == ESL_ERC_NO_ERROR) ? SECM_OK : SECM_NOT_OK;
    return result;
}
...
{
   vuint8 idx;
   vuint8 xorPadding[16u] = {0xD6,0xD7,0x98,0xFA,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF};
   vuint8 xorNvMPadding[16u] = {0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF};
   vuint8 rootKey[16] = {0};
   SecM_KeyType keyTemp = 0u;
   vuint8 keyArray[SEC_KEY_LENGTH];
   vuint8 seedByte[SEC_SEED_LENGTH];
   vuint8 computeKey[SEC_KEY_LENGTH];
   SecM_StatusType result = SECM_OK;

   /* Initialize local variables */
   result = SECM_OK;

   /* Access seed as byte array
   Remark: It has to be ensured that the handling is the same everywhere the seed is handled in byte context */
   seedByte[3] = (lastSeed.seedX & 0Xff);
   seedByte[2] = (lastSeed.seedX & 0Xff00) >> 8;
   seedByte[1] = (lastSeed.seedX & 0Xff0000) >> 16;
   seedByte[0] = (lastSeed.seedX & 0Xff000000) >> 24;

   if(SECM_OK == uds_PBKDF2GenerateRootKey(rootKey))
   {
       /* read the Xor and decrypt */
       if(SECM_OK == ApplFblNvReadEaUDSD_EncryptXORWritnVal(kFblWrapNvDefaultIndex, &xorNvMPadding[0]))
       {
           if((0xFF == xorNvMPadding[0]) && (0xFF == xorNvMPadding[1]) && (0xFF == xorNvMPadding[2]) && (0xFF == xorNvMPadding[3]))
           {
               ; /* Not write Xor yet, use the default XOR value */
           }
           else
           {
               /* Decrypt the value from NvM */
               (void)AES128_ECB_decrypt(xorNvMPadding, rootKey, xorPadding);
           }
       }
   }

   for(idx=0u; idx<SEC_KEY_LENGTH; idx++)
   {
       computeKey[idx] = seedByte[idx] ^ xorPadding[idx];
   }

   keyArray[0] = ((computeKey[0]&0x0f)<<4)|(computeKey[1]&0x0f);
   keyArray[1] = ((computeKey[1]&0xf0)>>4)|((computeKey[2]&0x0f)<<4);
   keyArray[2] = ((computeKey[2]&0xf0)>>4)|(computeKey[3]&0xf0);
   keyArray[3] = (computeKey[3]&0x0f)|((computeKey[0]&0xf0)>>4);

   keyTemp = (SecM_KeyType)((SecM_KeyType)keyArray[0] << 24) |
             (SecM_KeyType)((SecM_KeyType)keyArray[1] << 16) |
             (SecM_KeyType)((SecM_KeyType)keyArray[2] << 8) |
             (SecM_KeyType)((SecM_KeyType)keyArray[3]);

   /* Compare the decrypted seed */
   if (keyTemp != key)
   {
      result = SECM_NOT_OK;
   }
 ...
 **********************************************************************************************************************/
/* PRQA S 3673, 3677 1 */ /* MD_MSR_Rule8.13 */
SecM_StatusType SecM_CompareKeyLevel1( SecM_KeyType key, SecM_SeedType lastSeed )
{
   vuint8 idx;
   vuint8 xorPadding[16u] = {0xD6,0xD7,0x98,0xFA,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF};
   vuint8 xorNvMPadding[16u] = {0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF};
   ...
   if(SECM_OK == uds_PBKDF2GenerateRootKey(rootKey))
   {
       /* CR:8418158 read the Xor and decrypt */
       if(SECM_OK == ApplFblNvReadEaUDSD_EncryptXORWritnVal(kFblWrapNvDefaultIndex, &xorNvMPadding[0]))
       {
           if((0xFF == xorNvMPadding[0]) && (0xFF == xorNvMPadding[1]) && (0xFF == xorNvMPadding[2]) && (0xFF == xorNvMPadding[3]))
           {
   ...
   keyArray[0] = ((computeKey[0]&0x0f)<<4)|(computeKey[1]&0xf0);
   keyArray[1] = ((computeKey[1]&0x0f)<<4)|((computeKey[2]&0xf0)>>4);
   keyArray[2] = (computeKey[2]&0xf0)|((computeKey[3]&0xf0)>>4);
   keyArray[3] = ((computeKey[3]&0x0f)<<4)|(computeKey[0]&0x0f);
